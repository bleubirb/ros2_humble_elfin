...
class OnRobotRG2FTDriver(Node):
    ...
    def __init__(self):
        ...
        self.cmd_sub = self.create_subscription(
            RG2FTCommand, "gripper/ctrl", self.gripper.write_command, 10
        )

        self.restart_srv = self.create_service(Trigger, "restart", self.restart_cb)
        self.zero_srv = self.create_service(
            SetBool, "zero_force_torque", self.zero_force_torque_cb
        )
        self.set_prox_offset_srv = self.create_service(
            SetProximityOffsets, "set_proximity_offsets", self.prox_offsets_cb
        )
    ...
    def prox_offsets_cb(self, req, resp):
        ...
        self.gripper.set_proximity_offsets(
            req.proximity_offset_l, req.proximity_offset_r
        )
        resp.success = True
