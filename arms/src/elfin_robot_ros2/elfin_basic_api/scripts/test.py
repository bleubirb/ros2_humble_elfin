...
class test(object):
    def __init__(self):
        ...
        self.pub_joint_trajectory = self.node.create_publisher(
            JointTrajectory, "/joint_trajectory", 10
        )
        self.publisher_cart = self.node.create_publisher(PoseStamped, "/cart_goal", 10)
        self.publisher_cart_goal = self.node.create_publisher(
            PoseArray, "/cart_path_goal", 10
        )
        self.trajectory_cmd = self.node.create_publisher(
    ...
    def function_pub_joints(self):
        while True:
            ...
            self.node.get_logger().info(
                "Published joint positions: %s" % robot_state.position
            )
