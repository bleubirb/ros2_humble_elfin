...
LETTER_POSITIONS: dict[str, tuple[float, float, float]] = (
    {
        letter: (ORIGIN_X + x_rel / 1000, ORIGIN_Y + ROW1_Y / 1000, ORIGIN_Z)
        for letter, x_rel in LETTERS_ROW1_RELATIVE.items()
    }
    | {
        letter: (ORIGIN_X + x_rel / 1000, ORIGIN_Y + ROW2_Y / 1000, ORIGIN_Z)
        for letter, x_rel in LETTERS_ROW2_RELATIVE.items()
    }
    | {
        letter: (ORIGIN_X + x_rel / 1000, ORIGIN_Y + ROW3_Y / 1000, ORIGIN_Z)
        for letter, x_rel in LETTERS_ROW3_RELATIVE.items()
    }
    | {
        letter: (ORIGIN_X + x_rel / 1000, ORIGIN_Y + ROW4_Y / 1000, ORIGIN_Z)
        for letter, x_rel in LETTERS_ROW4_RELATIVE.items()
    }
    | {
        letter: (ORIGIN_X + x_rel / 1000, ORIGIN_Y + ROW5_Y / 1000, ORIGIN_Z)
